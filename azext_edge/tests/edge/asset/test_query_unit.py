...
from ....edge.common import ResourceTypeMapping
from ....edge.commands_assets import query_assets
...
def test_query_assets(mocked_cmd, build_query_fixture, req):
    ...
    if req.get("enabled"):
        expected_query += f"| where properties.enabled == {req['enabled']}"
    ...
    if req.get("endpoint_profile"):
        expected_query += f"| where properties.connectivityProfileUri =~ \"{req['endpoint_profile']}\""
