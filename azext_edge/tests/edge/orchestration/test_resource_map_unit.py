...
def _generate_records() -> List[dict]:
    return [{"id": generate_random_string(), "name": generate_random_string(), "apiVersion": generate_random_string()}]
...
@pytest.mark.parametrize(
    "expected_aio_extensions",
    [None, _generate_records()],
)
@pytest.mark.parametrize(
    "expected_aio_custom_locations",
    [None, _generate_records()],
)
@pytest.mark.parametrize("expected_aio_resources", [None, _generate_records()])
...
def test_resource_map(
    ...
    _assemble_connected_cluster_mock(
        ...
        extensions=expected_aio_extensions,
        custom_locations=expected_aio_custom_locations,
        resources=expected_aio_resources,
    ...
    _assert_ops_resource_eq(resource_map.extensions, expected_aio_extensions)

    mocked_connected_cluster().get_aio_custom_locations.assert_called_once()
    _assert_ops_resource_eq(resource_map.custom_locations, expected_aio_custom_locations)

    cl_count = 0 if not expected_aio_custom_locations else len(expected_aio_custom_locations)
    ...
    if expected_aio_custom_locations:
        for cl in expected_aio_custom_locations:
            _assert_ops_resource_eq(resource_map.get_resources(cl["id"]), expected_aio_resources)
    ...
    _assert_tree(
        ...
        expected_aio_extensions=expected_aio_extensions,
        expected_aio_custom_locations=expected_aio_custom_locations,
        expected_aio_resources=expected_aio_resources,
        expected_resource_sync_rules=expected_resource_sync_rules,
    )


def _assert_ops_resource_eq(actual: List[IoTOperationsResource], expected: List[dict]):
