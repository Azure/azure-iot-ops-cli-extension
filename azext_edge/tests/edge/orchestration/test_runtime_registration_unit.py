...
def invocation(mocker, tmp_path):
    ...
    def register(command_name="iot ops synthetic", validator=None, argument_validator=None, **metadata):
        group_name, verb = command_name.rsplit(" ", 1)
        with loader.command_group(
            group_name, command_type=CliCommandType(operations_tmpl=__name__ + "#{}")
        ) as group:
            group.command(verb, "synthetic_handler", validator=validator, **metadata)
