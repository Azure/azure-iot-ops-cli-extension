...
def test_schema_lifecycle(settings_with_rg, tracked_resources, tracked_files):
    ...
    # SHOW VERSION
    version_show = run(
        f"az iot ops schema version show -v {version_num} --schema {schema_name1} -g {registry_rg} "
    ...
    version_add = run(
        f"az iot ops schema version add -v {version_num2} --schema {schema_name2} -g {registry_rg} "
    ...
    # REMOVE VERSION
    run(
        f"az iot ops schema version remove -v {version_num} --schema {schema_name2} -g {registry_rg} "
        f"--registry {registry_name}"
    )
