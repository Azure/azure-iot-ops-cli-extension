...
import responses
from azext_edge.edge.commands_connector import add_connector_opcua_issuer, remove_connector_opcua_issuer, show_connector_opcua_issuer
...
def test_issuer_remove(
    ...
    if "cert3.der" in certificate_names and include_secrets:
        assert (
            mocked_logger.warning.call_args[0][0] == "Secret cert3-der not found in keyvault mock-keyvault. Skipping removal..."
        )
...
@pytest.mark.parametrize(
    "expected_resources_map, issuer_list_spc, issuer_list_secretsync, certificate_names, include_secrets, expected_secret_sync, expected_error",
    [
        # no cl resources
        (
            {
