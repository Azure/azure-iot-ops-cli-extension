...
from ..common import BROKER_RESOURCE, OPCUA_RESOURCE, CONSOLE_WIDTH, EdgeResourceType
...
def build_bundle(edge_resource: str, bundle_path: str, log_age_seconds: Optional[int] = None):
    ...
    # TODO: optimize
    if edge_resource == EdgeResourceType.auto.value:
        if get_cluster_custom_resources(resource=BROKER_RESOURCE):
            pending_work["e4k"].update(prepare_opcua_bundle(log_age_seconds))
        if get_cluster_custom_resources(resource=OPCUA_RESOURCE):
            pending_work["opcua"].update(prepare_opcua_bundle(log_age_seconds))
    elif edge_resource == EdgeResourceType.e4k.value:
        get_cluster_custom_resources(resource=BROKER_RESOURCE, raise_on_404=True)
        pending_work["e4k"].update(prepare_opcua_bundle(log_age_seconds))
    elif edge_resource == EdgeResourceType.opcua.value:
        get_cluster_custom_resources(resource=OPCUA_RESOURCE, raise_on_404=True)
        pending_work["opcua"].update(prepare_opcua_bundle(log_age_seconds))

    pending_work["shared"].update(prepare_shared_bundle())
    total_work_count = len(pending_work["opcua"]) + len(pending_work["e4k"]) + len(pending_work["shared"])
    ...
    with Live(grid, console=console, transient=True) as live:
        ...
        if pending_work["shared"]:
            visually_process(
                description="Processing common resources",
                support_segment=pending_work["shared"],
            )
