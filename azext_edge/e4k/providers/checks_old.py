...
def check_k8s_version():
    from azext_edge.e4k.constants import K8S_VERSION_MIN

    version_client = client.VersionApi()
    version_details = version_client.get_code()


    if version_details:
        version_dict = version_details.to_dict()
        major = version_dict.get("major")
        minor = version_dict.get("minor")
        patch = version_dict.get("patch")
        semver = f"{major}.{minor}{f'.{patch}' if patch else ''}"

        display_text = f"Kubernetes server version is {K8S_VERSION_MIN}"
        if version.parse(semver) >= version.parse(K8S_VERSION_MIN):
            nested_displays[0] = Padding(f":zap: {display_text}", (0, 0, 0, 4))
            # status = CheckTaskStatus.success.value
        else:
            nested_displays[0] = Padding(
                f"[red]:stop_sign:[/red] {display_text}. Please upgrade.", (0, 0, 0, 4)
            )



    display_key = f"Minimum Kubernetes server version {{expected>={K8S_VERSION_MIN}}}"
    nested_displays = []
    nested_displays.append(
        Padding(
            "[yellow]:warning:[/yellow] Unable to fetch Kubernetes server version.",
            (0, 0, 0, 4),
        )
    )
