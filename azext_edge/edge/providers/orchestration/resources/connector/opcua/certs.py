...
class OpcUACerts(Queryable):
    ...
    def trust_add(self, instance_name: str, resource_group: str, file: str, secret_name: Optional[str] = None) -> dict:
        ...
        # logger.info(f"Uploading {file} to keyvault {spc_keyvault_name} as secret {secret_name}.")
        ...
        if not opcua_spc:
            ...
            # logger.info("Azure Key Vault Secret Provider Class opc-ua-connector found, updating secret reference...")
    ...
    def _process_fortos_yaml(self, object_text: str, revert: bool = False, secret_entry: Optional[dict] = None) -> str:
        if object_text:
            object_text.replace("\n    - |", "\n- |")
    ...
    def issuer_add(
        ...
        if cert_extension not in ["der", "crt", "crl"]:
            raise ValidationError("Only der, crt and crl file extensions are supported.")
        ...
        if not opcua_spc:
            ...
            logger.info("Azure Key Vault Secret Provider Class opc-ua-connector not found, creating new one...")
            opcua_spc = {
                "location": self.instance["location"],
                "extendedLocation": self.instance["extendedLocation"],
                "properties": {
        ...
        # check if there is a secret sync called "aio-opc-ua-broker-issuer-list", if not create one
        secret_mapping = opcua_secret_sync.get("properties", {}).get("objectSecretMapping", [])
        # add new secret to the list
        secret_mapping.append(
            {
        ...
        if not opcua_secret_sync:
            logger.info("Secret Sync aio-opc-ua-broker-issuer-list not found, creating new one...")
            opcua_secret_sync = {
                "location": self.instance["location"],
                "extendedLocation": self.instance["extendedLocation"],
                "properties": {
            ...
            logger.info("Secret Sync aio-opc-ua-broker-issuer-list found, updating secret reference...")
            opcua_secret_sync["properties"]["objectSecretMapping"] = secret_mapping
