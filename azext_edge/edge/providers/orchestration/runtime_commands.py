...
class RuntimeCommandGroup(AzCommandGroup):
    ...
    def _show_command(self, name, getter_name="get", getter_type=None, custom_command=False, **kwargs):
        # Azure CLI's show/custom_show helpers bypass _command and reject extension
        # metadata during flattening, so consume and attach it on this path as well.
        requirement = kwargs.pop("runtime_requirement", None)
        parameters = tuple(kwargs.pop("runtime_parameters", ()))
        target = kwargs.pop("runtime_target", RuntimeTarget())
        super()._show_command(
            name, getter_name=getter_name, getter_type=getter_type, custom_command=custom_command, **kwargs,
        )
