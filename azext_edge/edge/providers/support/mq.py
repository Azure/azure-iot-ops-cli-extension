...
MQ_FILE_PATH = MQ_ACTIVE_API.moniker
...
def fetch_diagnostic_metrics(namespace: str):
    ...
    try:
        ...
        return {
            "data": stats_raw,
            "zinfo": f"{namespace}/{MQ_FILE_PATH}/diagnostic_metrics.txt",
        }
    except Exception:
    ...
    for namespace in namespaces:
        try:
            ...
            if traces:
                for trace in traces:
                    zinfo = ZipInfo(
                        filename=f"{namespace}/{MQ_FILE_PATH}/traces/{trace[0].filename}",
...
def fetch_deployments():
    processed, namespaces = process_deployments(
        file_path=MQ_FILE_PATH, label_selector=MQ_LABEL, return_namespaces=True
    )
    # aio-mq-operator deployment has no app label
    operators, operator_namespaces = process_deployments(
        file_path=MQ_FILE_PATH, field_selector=f"metadata.name={AIO_MQ_OPERATOR}", return_namespaces=True
    )
    processed.extend(operators)

    operators_v2, operator_namespaces_v2 = process_deployments(
        file_path=MQ_FILE_PATH, label_selector=MQ_NAME_LABEL, return_namespaces=True
    )
