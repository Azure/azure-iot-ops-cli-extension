...
def fetch_deployments():
    processed, namespaces = process_deployments(
        file_path=MQ_ACTIVE_API.moniker, label_selector=MQ_LABEL, return_namespaces=True
    )
    # aio-mq-operator deployment has no app label
    operators, operator_namespaces = process_deployments(
        file_path=MQ_ACTIVE_API.moniker, field_selector=f"metadata.name={AIO_MQ_OPERATOR}", return_namespaces=True
    )
    processed.extend(operators)

    operators_v2, operator_namespaces_v2 = process_deployments(
        file_path=MQ_ACTIVE_API.moniker, label_selector=MQ_NAME_LABEL, return_namespaces=True
    )
...
def fetch_statefulsets():
    processed = process_statefulset(
        file_path=MQ_ACTIVE_API.moniker,
